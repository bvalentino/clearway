...
| File | State |
| --- | --- |
| `Sources/App/WorkflowDefinition.swift` | Deleted |
| `Sources/App/WorkflowLoopEngine.swift` | Deleted |
| `Sources/App/WorkTaskCoordinator+WorkflowEngine.swift` | Deleted (took `WorkflowCountdown`, `WorkflowLaunchID`, `WorkflowAdvanceResult` with it) |
| `Tests/WorkflowAgentLaunchTests.swift` … `Tests/WorkflowSidebarActionTests.swift` | All ten engine test files deleted |
| `Sources/App/WorkTaskCoordinator.swift` | Reduced to `pendingLaunch`, the three dependencies, `startTask`, `completePendingLaunch`, `worktreeForTask` and `ensureShadowTask`. The engine block, both definition caches + `refreshWorkflowJSONGate`, `workflowAgentCommand`, `workflowAfterCreateHook`, the whole agent-surface subsystem (`agentSurfaces`, `setAgentSurface`, `agentSurfaceIdentities`, `launchPromptFiles`, `isAgentSurface`, `handleMainTabClosed`, `shouldClearLiveAgentState`, `handleChildExited`, `exitObserver` + its `.ghosttyChildExited` registration), the `onTasksReloaded` / `onClearwayChanged` wiring and the gate seed are gone; `startTask` now writes `status = in_progress`; the type's, `startTask`'s and `completePendingLaunch`'s doc comments rewritten |
| `Sources/App/ContentView.swift` | `seedWorkflowStatus` call + comment removed; the after-create hook now runs the project hook alone (no `chainCommands`); `appProvider` wiring removed; `handleWorktreeRemoved` call and its `weak workTaskCoordinator` capture removed |
| `Sources/App/ProjectWindow.swift` | `skipAutoRestart` / `onMainTabClosed` wiring removed from `onAppear` |
| `Tests/TestHelpers.swift` | `WorkflowHarnessTestCase` deleted; the now-unused `import GhosttyKit` dropped. `makeWorktree` and `TempRootTestCase` kept |
| `Tests/WorkTaskCoordinatorTests.swift` | Raw/validated cache tests and their `makeCoordinator(workflowJSON:)` helper deleted; `testHandleWorktreeRemovedDoesNotResurrectTaskCentrally` deleted with the method; new `testStartTaskMovesABacklogTaskToInProgress`; `testStartTaskUsesFreshDiskContentNotStaleSnapshot`'s status assertion retargeted to `in_progress` |
| `Clearway.xcodeproj/project.pbxproj` | Regenerated by `xcodegen generate` for the thirteen deletions |

**Evidence: both decision-1 pins watched failing against the unfixed `startTask`**

With `updated.errorMessage = nil` temporarily restored in place of the `in_progress` write and
`-only-testing:ClearwayTests/WorkTaskCoordinatorTests`:

```
Tests/WorkTaskCoordinatorTests.swift:37: error: -[ClearwayTests.WorkTaskCoordinatorTests testStartTaskMovesABacklogTaskToInProgress] : XCTAssertEqual failed: ("Optional("ready_to_start")") is not equal to ("Optional("in_progress")") - Start Now advances a backlog task to in_progress
Tests/WorkTaskCoordinatorTests.swift:88: error: -[ClearwayTests.WorkTaskCoordinatorTests testStartTaskUsesFreshDiskContentNotStaleSnapshot] : XCTAssertEqual failed: ("Optional("ready_to_start")") is not equal to ("Optional("in_progress")")
** TEST FAILED **
```

The `in_progress` write was restored immediately afterwards. Everything else in this task is deletion,
for which the compiler is the bar: the engine's declarations no longer exist, so any surviving reader
is a build error.

**Deviations from the plan**

- **`handleWorktreeRemoved` was deleted, not emptied.** The plan said to keep "whatever teardown is not
  agent-surface bookkeeping" — there was none: every line of the body was either surface bookkeeping or
  engine state (`runningAction`, `engineHalted`, `lastKnownAutopilot`, `cancelCountdown`). What remained
  was a function that resolved a worktree and did nothing with it, still called from
  `ContentView.beginRemoveWorktree`. The method, that call site and the `weak workTaskCoordinator`
  capture went with it, and `WorkTaskCoordinatorTests`' `testHandleWorktreeRemovedDoesNotResurrectTaskCentrally`
  with them: its contract (removing a worktree writes nothing back centrally) is now structural — the
  coordinator has no worktree-removal path at all.
- The after-create hook block keeps the plan's `if let cmd = projectHookCmd, …` shape but its comment
  lost the "in parallel … decoupled from the agent launch" clause, which described a launch that no
  longer happens.
- `WorkTaskCoordinator`'s file-level doc comment ("Coordinates the task launch workflow … and launching
  the configured agent") was rewritten rather than trimmed; nothing on the type launches an agent now.
- `Tests/TestHelpers.swift`'s `import GhosttyKit` was dropped — only `WorkflowHarnessTestCase`'s
  `dummyApp` needed it. `WorkTaskCoordinatorTests` declares its own `dummyApp` and imports GhosttyKit
  itself.
- `TerminalManager.launchAgentTab` and `TerminalManager+TaskTerminals.terminateSurface` are now
  callerless in `Sources`. Left in place: T9 owns the terminal layer. T9's brief names `terminateSurface`
  but **not** `launchAgentTab` — see Follow-ups.

**Gate:** `./scripts/ci.sh` — passed, exit 0. `Executed 337 tests, with 0 failures (0 unexpected)`.
...
| File | State |
| --- | --- |
| `Tests/AgentLaunchAgentTests.swift` | Deleted |
| `Tests/AgentLaunchModelTests.swift` | Deleted |
| `Sources/App/AgentLaunch.swift` | `isAllowlistedAgentCommand`, `allowlistedAgent`, `resolveAgentCommand`, `agentsAcceptingModelFlag`, `applyModel`, `isModelValueSafe` and `acceptsModelFlag` removed. Reduced to `agentAllowlist` and `buildAgentPromptCommand`; the allowlist's doc comment now names only the Settings → Main Terminal picker rows, and the ARG_MAX note names the prompt launcher instead of "launcher / Plan / workflow prompts" |
| `Sources/App/WorktreeHooks.swift` | `chainCommands` removed (T6 removed its only call site) |
| `Tests/WorktreeHooksTests.swift` | The four `chainCommands` tests and their `// MARK: - Hook chaining` heading removed |
| `Clearway.xcodeproj/project.pbxproj` | Regenerated by `xcodegen generate` for the two deletions |

**Evidence**

A pure deletion with no behavioural branch left to exercise, so no regression test was written: there
is nothing that could pass before the change and fail after it. The compiler is the bar —
`resolveAgentCommand`, `applyModel`, `isModelValueSafe`, `isAllowlistedAgentCommand` and
`chainCommands` no longer exist, so any surviving reader is a build error. The two surviving
declarations keep their own pins: `agentAllowlist` is read by `SettingsView.swift:11` (the picker's
`ForEach`), and `buildAgentPromptCommand` by `TerminalManager+Launcher.swift:38` and the six tests in
`TerminalManagerTests.swift:93-175`, which include the shape invariants
(`writesPromptFile_andQuotesAgentCommand`, `keepsSpecialCharsInFile_notInShellString`,
`escapesSingleQuotes_inAgentCommand`). Deleting `AgentLaunchModelTests.swift` therefore loses no
coverage of `buildAgentPromptCommand`.

**Deviations from the plan:** none.

**Gate:** `./scripts/ci.sh` — passed, exit 0. `Executed 305 tests, with 0 failures (0 unexpected)`.
