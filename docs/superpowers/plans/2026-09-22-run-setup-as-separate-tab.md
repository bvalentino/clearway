...
| File | State |
| --- | --- |
| `Sources/App/ContentViewHelpers.swift` | `hookShellCommand(_ cmd: String, path: String = ShellEnvironment.path)` exports `path`; the rest of the body is unchanged. |
| `Tests/HookShellCommandTests.swift` | New: the exact wrapped line for `/opt/bin:/usr/bin`, and for a path containing a single quote. |
| `Clearway.xcodeproj/project.pbxproj` | Regenerated by `xcodegen` to include the new test file. |

**Evidence.** RED: `./scripts/ci.sh` with only the test file added exited 65:

```
Tests/HookShellCommandTests.swift:12:57: extra argument 'path' in call
Tests/HookShellCommandTests.swift:19:51: extra argument 'path' in call
```

GREEN: the xcresult lists `HookShellCommandTests/testExportsTheGivenPathAroundTheHook()` and
`testEscapesASingleQuoteInThePath()` as Passed. Criterion 2: `git diff --stat -- Sources/App/ContentView.swift`
prints nothing.

**Deviations.** Criterion 1 lists substrings (prefix, `export PATH=`, `(make setup)`, the banner).
The tests assert the whole wrapped line instead, which pins every one of them and the unchanged
...
| File | State |
| --- | --- |
| `Sources/App/TerminalManager.swift` | `appendTab(for:app:command:name:activate:)` builds the tab with `name`; sets `activeId` and calls `transferFirstResponder` only when `activate`; records `isFirstTab` (`existingPane?.main.tabs.isEmpty ?? true`) before the append and, after `objectWillChange.send()`, opens the Setup tab when `takeSetupHook(for:)` returns a hook. Doc comment rewritten. |
| `Sources/App/TerminalManager+Setup.swift` | New: `openSetupTab(for:app:hook:)` appends through `appendTab(name: "Setup", activate: false)`, then a `Task { @MainActor }` does `awaitPath()` → `awaitShellPrompt(on:)` → `sendPaste(hookShellCommand(hook, path:))`. |
| `Sources/App/TerminalManager+Panels.swift` | `runHookInSecondary`, `revealSecondaryForHook`, their MARK header, and the now-unused `import GhosttyKit` removed. |
| `Sources/App/ContentView.swift` | The post-create handler no longer runs the hook in the secondary panel; `projectHookCmd` still goes to `markWorktreeCreated(setupHook:)`. |
| `Tests/TerminalManagerTests.swift` | `test_runHookInSecondary_forcesSecondaryVisible_overridingOpenOnStartOff` and its MARK header removed. |
| `Clearway.xcodeproj/project.pbxproj` | Regenerated by `xcodegen` to include the new source file. |

**Evidence.** No new test: every rule this task adds lives inside `appendTab` / `openSetupTab`, which
need a `ghostty_app_t` XCTest cannot produce (the plan assigns ordering and non-activation to the
operator). The testable pieces (display title, `hookShellCommand(path:)`, `pendingSetupHooks`,
panel independence) were pinned in T1-T3. Criterion 1:
`grep -rn "runHookInSecondary\|revealSecondaryForHook" Sources Tests` hits only
`Sources/Ghostty/CLAUDE.md:6`, which T5 owns. Criterion 2: `grep -rn "TerminalTab(" Sources/App` hits
only `TerminalManager.swift:367`, inside `appendTab`. Criteria 3-5 by reading the diff: `openSetupTab`
touches none of `secondaryVisible`, `pane.secondary`, `activeId`, `transferFirstResponder`; the line
is built after `awaitPath()` and sent after `awaitShellPrompt`; the new parameters default to
`name: nil, activate: true`, and no existing caller was edited.
