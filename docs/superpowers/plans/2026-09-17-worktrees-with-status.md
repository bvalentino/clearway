...
| File | State |
| --- | --- |
| `Sources/App/WorktreeRow.swift` | New. `WorktreeRow`, `ShortcutBadge` and `PrimaryBadge` moved verbatim, plus `StatusBadge` and `WorktreeRow.status`. |
| `Sources/App/SidebarView.swift` | 668 → 579 lines; the three moved structs deleted, nothing else touched. |
| `Clearway.xcodeproj/project.pbxproj` | Regenerated by `xcodegen` for the new file. |

`ShortcutBadge` is now internal — `SidebarView.destinationRow` still constructs it. `PrimaryBadge`
and the new `StatusBadge` stay `private` to the new file. `WorktreeRow.status` defaults to `nil`, so
the one construction site in `worktreeRowView` compiles unchanged and no row shows a badge yet.
The badge slot is `if worktree.isMain { PrimaryBadge() } else if let status { StatusBadge(...) }`;
main can carry no status, so the branches cannot contest the slot.

**Evidence**

No new test. The move changes no behaviour and a SwiftUI `View` body is not reachable from XCTest,
so the existing suite passing is the proof the plan asks for — there is no unfixed code for a
regression test to fail against.
...
`./scripts/ci.sh` — green after the last edit: `Executed 449 tests, with 0 failures`, `==> CI passed.`
...
| File | State |
| --- | --- |
| `Sources/App/SidebarView.swift` | 655 → 679 lines. `worktreeContextMenu` gains the `Menu("Status")` inline picker for a non-main worktree, in its own block between the two destructive actions and the Open in / Reveal / Copy block. `worktreesSectionHeader` becomes a `@ViewBuilder` that attaches its `dropDestination` only outside `.none`, routing through the new `dropIntoWorktreesHeader`. `statusSection`'s header is a targeted drop destination driven by a new `@State targetedStatus`. `worktreeRowView` gates `draggableIf` on `grouping != .none`. New `dropIntoWorktreesHeader` and `applyStatus(_:to:)`. The gear menu moved out. |
| `Sources/App/GroupByMenu.swift` | New. The Worktrees header's gear — the Group by picker plus the Worktree Settings item — lifted verbatim out of `SidebarView`, reading `groupManager` from the environment and taking the sheet as a closure. |
| `Clearway.xcodeproj/project.pbxproj` | Regenerated by `xcodegen` for the new file. |

`moveDisabled` needed no change: T6 already passes `true` for every row outside the `.group`
branch, and `.onMove` is declared only there, so there is no reorder in `.status` or `.none` to
disable.

The status header gets `.frame(maxWidth: .infinity, alignment: .leading)` so its drop zone spans
the header the way a group header's `HStack` + `Spacer()` already does; a bare `Text` would accept
a drop only over the word itself. `applyStatus` resolves each dropped ID against
`worktreeManager.worktrees` and defers through `DispatchQueue.main.async`, the same shape
`dropIntoGroup` uses to stay out of the `NSTableView` drop delegate.

**Deviation from the plan**

The gear menu was extracted, which the plan listed as the mitigation to apply if `SidebarView.swift`
crossed SwiftLint's 700-line `file_length` warning. T7's additions put it at 706 lines; moving
`groupByMenu` into `Sources/App/GroupByMenu.swift` brings it to 679. No `swiftlint:disable` was
added, as the spec forbids.

**Evidence**

No new test. Every acceptance criterion here is a gesture or view state — which menu item renders,
which header accepts a drop, whether a row can be picked up — and none of it is reachable from
XCTest. The mutation those gestures drive is `setStatus` and its main-worktree guard, already
pinned by T3's watched failures. The criteria are confirmed by hand against `./scripts/run.sh`.
