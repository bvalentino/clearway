...
| --- | --- | --- | --- |
| 1 | What sets a task's status once a worktree exists? | **Start Now** writes `status = in_progress`. No status picker is restored in the task aside; a worktree task then sits on `in_progress` with no in-app way to change it. The status badges and the labels for the legacy fixed states stay. (The Ready to Start toggle was kept here and later removed — see decision 13; the badges and labels were later removed too — see decision 14.) | Operator |
| 2 | What happens to `autopilot` and `completed` in `TASK.md`? | Both leave the `WorkTask` model and the frontmatter entirely, so they vanish from a `TASK.md` on its next write. An unknown `status` slug left behind by an old workflow is carried through untouched (it was displayed as-is via `humanize`; nothing displays status at all after decision 14). `.clearway/WORKFLOW.json` is simply never read again: no warning, no migration, no deletion. | Operator |
...
| 13 | Does the Ready to Start toggle survive? | No — added after T1–T10 landed, during the hands-on check. The toolbar toggle, the backlog row's context-menu item, `WorkTaskCard.onReadyToStart` and the standalone window's `Start Now` / `Ready to Start` menus all go; a task moves from the backlog straight to `in_progress` via **Start Now**. Nothing then writes `ready_to_start`, so `ReservedStatus.readyToStart` (with `backlogMarkers`, its only remaining reader), its "Ready to Start" label, its indigo badge colour and checkmark glyphs, its `startTask` gate arm and `WorkTaskManager.setStatus` (production-dead once the four toggle writers go) are removed too. A `TASK.md` still carrying `ready_to_start` falls under decision 2 — humanized to "Ready To Start" with the green badge — and, being no longer a startable status, its **Start Now** is a no-op, the same as for any other unknown slug. (That last part is amended by decision 17: the slug is now migrated to `new` on load, so such a task is startable again.) | Operator |
| 14 | Is a task's status ever rendered? | No. Nothing in the app displays it, so the badge and the label table go: `WorkTaskStatusBadge`, `badgeColor(for:)`, `WorkTaskCard.showStatusBadge`, `WorkTask.displayLabel(for:)` and `humanize`, plus the `ReservedStatus` values left without a reader (`qa`, `readyForReview`, `done`). `status` stays a frontmatter field Clearway writes (`new` on create, `in_progress` on Start Now) and round-trips; `canceled` survives as a read-only constant — `migrateStatus` maps the legacy `stopped` onto it and `startTask` treats it as restartable; an unknown slug is carried through untouched with no label to derive. **Supersedes decisions 1 and 2** where they said the badges and the humanized labels stay. | Operator |
| 15 | Does the Ghostty readiness guard on Start Now survive? | No. `ContentView.startWorkTask`'s `guard ghosttyApp.app != nil else { return }` was the last trace of `startTask` taking a `ghostty_app_t`; the simplify pass reduced it to an argument-free readiness check, and `startTask` neither takes nor touches a terminal. The guard now only makes Start Now silently no-op when the app handle failed to build. A future base-branch/fetch feature will add its own gate if it needs one. | Operator |
| 16 | What happens to the pre-existing dead code the review found? | Removed in this change, not deferred: `TerminalManager.newShellTab`, `YAMLHelpers.replacingBody` and `WorkTask.replacingTitle` each have zero callers across `Sources/` and `Tests/` (re-verified by grep before deletion). They predate this branch, but leaving verified-dead code behind a change whose objective is "no dead seam" is not worth a separate task. | Operator |
| 17 | What happens to a `TASK.md` a shipped version left on `ready_to_start`? | It migrates to `new` on load. `migrateStatus` gains a `ready_to_start` arm beside the existing retired-slug arms, so the task parses as a backlog task, is startable via **Start Now**, and writes the migrated slug through on the next save like `open` / `started` / `stopped` do. Without it such a task is permanently unstartable with no feedback: Start Now renders enabled, `startTask` returns `.ignored`, and the badge decision 13 relied on to at least show the state was deleted by decision 14. **Amends decision 13.** | Operator |
...
| `CLAUDE.md` `## Verifying a change` and `## Pipeline` sections | Both name `.github/workflows/ci.yml` by path; the latter also documents the `/work` pipeline for this repo. Unrelated to the app feature. |
| `Tests/WorkTaskTests.swift` | `testRetiredFieldsAreDroppedOnReserialize` (success criterion 4) must name `autopilot` / `completed` / `error_message` to assert they are dropped. |
...
`Tests/WorkTaskTests.swift`'s `testRetiredFieldsAreDroppedOnReserialize` is the one permitted
`error_message` hit, for the reason given in the table above.)

`Clearway.xcodeproj/project.pbxproj` is **tracked** and so is inside the audit, but it is regenerated
by `xcodegen generate` and must never be hand-edited — commit whatever the regeneration writes. The
ignored paths `.clearway/` and `.work/` are outside the audit by construction: `.clearway/TASK.md` and
`.work/state.json` are live runtime state. Confirm separately that
