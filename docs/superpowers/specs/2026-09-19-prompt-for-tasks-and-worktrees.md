...
| 4 | How is a backlog task planned? | A "Plan" menu on the task list toolbar and on the row context menu. Its primary action runs the plan default; its dropdown lists every agent command, and choosing one runs it **and** makes it the default immediately. Disabled when the project has no agent commands. It runs in the **primary** worktree (`isMain`) as a launcher tab through `TerminalManager.run`, submitted or staged per the command's `autoRun`. Task status is not changed. | Operator |
...
| 8 | Where do the defaults live? | `<projectPath>/.clearway/command-defaults.json`, a sibling of `commands.json`, read and written by `SavedCommandStore` and held by `SavedCommandManager`. Two slots, `afterCreate` and `plan`, each an optional command id. `commands.json` stays a plain array — the defaults are not part of the list and putting them in it would change its wire format. | Operator |
...
| 11 | What does the Plan button do with no default set? | Opens the menu. The `Menu` is declared without `primaryAction:` while the plan default resolves to nil, so the first click always shows the list; once a default exists the primary action runs it. Falling back to "the first agent command" would run something the user never chose. | Spec |
