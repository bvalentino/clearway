...
| 24 | What happens to a task when its worktree create fails? | It is put back exactly as it was. `confirmCreate` writes `status = in_progress` and `worktree = <branch>` before `git worktree add` runs, so a failure left the task naming a branch with no worktree — a state `resolveStart` refuses, making it unstartable from the UI. `PendingCreate` now carries the prior `status`, `worktree` and `attempt`, and `abandonPendingCreate()` restores them and clears the pending create. The sheet calls it on both `.reportedFailure` and `.silentFailure` and stays open. | Operator (review) |
| 25 | What happens when a plan would replace a running agent? | The operator confirms first. `planTask` opens a fresh surface over whatever the task terminal holds, so a live foreground process would be taken away silently. `WorkTaskListView` presents a destructive Replace / Cancel `confirmationDialog` — the shape `SidebarView`'s worktree-remove uses — when `TerminalManager.taskHasActiveProcess` is true, and plans immediately otherwise. The rule itself is `WorkTaskCoordinator.planNeedsConfirmation(hasActiveProcess:)`, pure and tested. Cmd+J's own replacement of a running terminal is untouched; it is a pre-existing follow-up. | Operator (review) |
| 26 | What clears the task selection after a successful start? | `ContentView`'s `onChange(of: worktreeManager.lastCreatedBranch)` handler, just before it consumes the pending create. The started task's file relocates into the new worktree and so leaves the backlog; a selection still naming it left the Tasks toolbar and Start Now's menu acting on a task that is no longer there (the base `.reuse` branch cleared it, the `.prefill` branch did not, and nothing cleared it after Create). That handler is the single point every successful create lands on, whichever door opened the sheet, and it already owns the navigation that follows. It clears only when the pending create's branch matches and its task id is the selection, so a create for some other task leaves the selection alone. | Spec (placement) + Operator (review) |
| 27 | What is `buildAgentPromptLine`'s actual safety contract? | Not `buildAgentPromptCommand`'s. There is no `$1` and no parameter expansion on that path: the agent command is concatenated into a line that `sendText` stages on an interactive prompt, and the operator's own shell parses it as source if they press Enter. The true property is that the line is staged, visible and user-owned rather than run — the same contract as `buildOpenInScript`. Only the file path is escaped, because Clearway chose it. The doc comment and the `CLAUDE.md` sentence said otherwise and were corrected. | Operator (review) |
| 28 | Where does the "Run after create" command run? | It **is** the new worktree's first tab, not a tab beside it. Main's `markWorktreeCreated` already claims a created worktree's first tab for the Settings → Main Terminal agent, so running the pick from `ContentView`'s `lastCreatedBranch` handler as well opened two agents. The mark now carries the pick, and `TerminalManager.firstTabSource(afterCreateCommand:mainCommand:)` — pure, `static`, tested — answers pick, else Main Terminal agent, else login shell; `pane(for:)` is the one place that reads it, so one code path builds the first tab. With no pick the behaviour is main's, unchanged. The afterCreate *hook* in the secondary panel is separate and untouched. | Operator (hands-on check) |
