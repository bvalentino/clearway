...
| 22 | What does "waiting on permission" look like? | The existing blue 7 pt dot, tooltip "Waiting for permission". No new colour: blue is already the row's "needs you" mark, the brief retires it for agent tabs, and two hues within one hue family at 7 pt are not distinguishable. Working keeps the orange pulsing dot unchanged. Precedence in `WorktreeRow` becomes waiting, then working, then the plain-shell notification. | Spec |
...
| 25 | Is the subagent's description shown? | No. `SubagentStart` delivers `agent_type` and `agent_id` and nothing else (Assumption 6), on both agents. Correlating a description out of the `Agent`/`Task` tool's `PreToolUse` would mean guessing which pending call a later `agent_id` belongs to. The brief accepts type alone. | Operator (brief, "Type alone is acceptable") + Spec (verified) |
