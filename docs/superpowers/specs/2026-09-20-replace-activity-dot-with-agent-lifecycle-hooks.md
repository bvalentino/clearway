...
| 20 | What are the transitions? | `SessionStart` → reset the surface entry to idle with an empty roster. `UserPromptSubmit` → working, clear the lead tool. `PreToolUse` with no `agent_id` → working, lead tool = `tool_name`; with an `agent_id` → upsert that subagent from `agent_id`/`agent_type`, surface working, the tool name discarded because only the lead's is rendered (change C5). `PostToolUse` → clear the lead tool when the event names no subagent, and nothing at all when it names one, stay working. `PermissionRequest` → waiting on permission, recording `tool_name` under the same rule. `SubagentStart` → upsert the subagent from `agent_id`/`agent_type`. `SubagentStop` → remove that subagent. `Stop` → idle, clear the lead tool **and the whole roster**, so a missed `SubagentStop` cannot pin a row. `SessionEnd` → drop the surface entry. Any event for a surface id that was retired in this process is ignored. | Operator (brief) + Spec |
...
| 24 | How do subagent rows render? | As extra views inside the existing `ForEach` body, after the worktree row: no `.tag`, so they are not selection destinations (the precedent is the search, loading and error rows in `worktreesSection`); `.moveDisabled(true)`; no leading padding, so a row shares the worktree row's left edge (change C1). One line, a `Label` over the sidebar's shared icon slot, so the text starts on the worktree row's title column (change C6): the agent type, with the description beside it when there is one (changes C4, C5). The slot carries a drawn `└` — `SidebarChildConnector`, a 1 pt `.secondary` stroke from the top of the slot to its centre and out to its trailing edge, centred in the slot — the connector a terminal draws before a child line. It is a `Path` and not the character: `└`'s shape belongs to the font, and the sidebar's is proportional. It replaced `point.3.connected.trianglepath.dotted` (change C3), which read as a node graph rather than as nesting. No `OutlineGroup`, no `DisclosureGroup` — the sidebar has neither today. | Operator (changes C1, C3, C5, C6) + Spec |
| 25 | Is the subagent's description shown? | Yes, when a `Stop` supplies one — beside the type on the same line, the way Claude Code's own status line writes the pair (`general-purpose  Count Swift files slowly`). It is the row's whole second half: change C5 removed the in-flight tool that sat below it, and the store no longer records a subagent's tool at all. `SubagentStart` carries no description and the correlation out of the `Agent` tool's `PreToolUse` this originally rejected is still rejected, but the captured payloads of change C4's predecessor show `Stop`'s `background_tasks` entries carry `description` beside `id`/`type`/`status`/`agent_type`. It is stored on the roster row and a later event that omits it never blanks it, so the row keeps the summary for the subagent's whole life. Type alone remains what a row falls back to. | Operator (change C4) + Spec (captured payload) |
...
- Spawning a subagent adds a non-selectable child row under the worktree naming its type, and its
  description once a `Stop` supplies one; the row goes on that subagent's `SubagentStop`, and any
  row a later `Stop` no longer reports as running goes with it.
