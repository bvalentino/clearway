...
| 24 | How do subagent rows render? | As extra views inside the existing `ForEach` body, after the worktree row: no `.tag`, so they are not selection destinations (the precedent is the search, loading and error rows in `worktreesSection`); `.moveDisabled(true)`; no leading padding, so a row shares the worktree row's left edge (change C1). One line, a `Label` over the sidebar's shared icon slot carrying `point.3.connected.trianglepath.dotted` (change C3): the agent type, with the description beside it when there is one (changes C4, C5). No `OutlineGroup`, no `DisclosureGroup` — the sidebar has neither today. | Operator (changes C1, C3, C5) + Spec |
