...
plus, for the one case that leaves the user's change half-applied on disk, an `NSAlert`.
...
| 2 | Does any other failed write alert? | No. Individual `clearway.group`, `clearway.position` and `clearway.grouping` writes, and the `replaceLocalValues` registry write when it fails on its own, are log-only. Each is a single value the next gesture overwrites; only the abandoned registry leaves the change half-applied. | Operator |
...
| 12 | What does the alert say? | Title `Couldn't save the group "<group>"`; body `Clearway couldn't write the group for <path>, so the sidebar will go back to how it was.` One sentence, and it names the revert, because the unexplained revert is the reported symptom. | Spec author |
...
7. **The one test that builds a manager outside the base cannot reach the alert.** `testAProjectWhereTheExtensionCannotBeEnabledShowsNoGroups` (`Tests/WorktreeGroupPersistenceTests.swift:238-260`) calls only `createGroup`, which passes no members to `writeRegistry` (`Sources/App/WorktreeGroupManager.swift:82-86`), so it reaches the log-only registry failure and never the presenter.
...
- Body: `Clearway couldn't write the group for <path>, so the sidebar will go back to how it was.`
