...
| 13 | Does anything else change? | No. The five cases, their slugs, their display names, their colours, the persisted format, the grouping rules, the drop targets and the `.status`-mode badge suppression (`SidebarView.swift:504`) are all untouched. | Spec author |
