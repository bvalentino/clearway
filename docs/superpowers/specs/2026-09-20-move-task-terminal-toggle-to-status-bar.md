...
| 3 | Who owns the action after the move? | `TaskDetailView` itself. It gains one `@EnvironmentObject private var workTaskCoordinator: WorkTaskCoordinator` and calls `workTaskCoordinator.toggleTaskTerminal(taskId: taskId, app: app)` behind a `guard let app = ghosttyApp.app`. It is **not** parameterised the way `WorktreeStatusBar` takes `secondaryVisible` / `onToggleSecondary`: that view is a standalone struct that does not know which worktree it draws, so the state had to be handed in, whereas `TaskDetailView` already holds `taskId` (line 15), `terminalVisible` (lines 35-37) and `ghosttyApp` (line 10). Parameterising would also mean a new private method on `ContentView`, which is at 1014 lines against SwiftLint's 1000-line `file_length` error and only builds on its file-wide disable — CLAUDE.md says the next addition there needs a split first. This change adds nothing to `ContentView`. | Spec author |
| 4 | How does the toolbar item's `.disabled(selectedTask == nil \|\| ghosttyApp.readiness != .ready)` carry over? | The `selectedTask == nil` half becomes structural and disappears: `pathBar` is only reachable inside `body`'s `if let task` (`TaskDetailView.swift:46`, `121`), so a task always exists where the button renders. The readiness half survives verbatim as `.disabled(ghosttyApp.readiness != .ready)` on the button. | Spec author |
| 5 | Why `readiness` and not `ghosttyApp.app` for that gate? | `readiness` is `@Published` (`Ghostty.App.swift:25`); `app` is a plain computed property over `appHandle` with no change to publish (`Ghostty.App.swift:31`), so a view gated on it has nothing to re-evaluate against when the app comes up. This is the same reason CLAUDE.md records for the sibling toolbar buttons and for `startNowItems`. The `guard let app = ghosttyApp.app` stays inside the action, where the launch actually needs the pointer — the same split as `plan`. | Spec author |
| 6 | Disabled or omitted while readiness is not `.ready`? | Disabled. CLAUDE.md's Open-in rule draws the line: the toolbar item disappears on an empty list because "an empty list is a configuration the user chose, not a momentarily unavailable action". A terminal that has not finished coming up is exactly a momentarily unavailable action, so the control stays put and greys. A button that materialises a second after the window opens would also read as a glitch in a bar whose only other content is static text. | Spec author |
...
| 17 | Are any docs stale after this? | Two lines, both corrected in the same change. `CLAUDE.md:235` calls the last capsule "the copy/terminal/`…` group" — it becomes the copy/`…` group. `WorkTaskCoordinator+TaskTerminal.swift:10-11` says "Cmd+J passes `true`, the toolbar button `false`" — it becomes the status bar button. PR #221 touched no docs because no sentence described the toolbar it changed; these two do. | Spec author |
...
5. While `ghosttyApp.readiness` is not `.ready` the button is present and disabled, and becomes
...
| File | Change |
| --- | --- |
| `Sources/App/TaskDetailView.swift` | Add `@EnvironmentObject private var workTaskCoordinator: WorkTaskCoordinator`, a `terminalToggleLabel` computed property and a `toggleTerminal()` private method guarding `ghosttyApp.app`; add the button after `pathBar`'s `Spacer()` with `.buttonStyle(.plain)`, `.help`, `.accessibilityLabel`, `.pointerCursorOnHover()` and `.disabled(ghosttyApp.readiness != .ready)`. |
...
| `Sources/App/WorkTaskCoordinator+TaskTerminal.swift` | One doc-comment phrase: "the toolbar button `false`" becomes "the status bar button `false`". |
| `CLAUDE.md` | One phrase at line 235: "the copy/terminal/`…` group" becomes "the copy/`…` group". |
