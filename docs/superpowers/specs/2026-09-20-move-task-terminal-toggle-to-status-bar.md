...
| File | Change |
| --- | --- |
| `Sources/App/TaskDetailView.swift` | Add `@EnvironmentObject private var workTaskCoordinator: WorkTaskCoordinator`, a `terminalToggleLabel` computed property and a `toggleTerminal()` private method guarding `ghosttyApp.app`; add the button after `pathBar`'s `Spacer()` with `.buttonStyle(.plain)`, `.help`, `.accessibilityLabel` and `.pointerCursorOnHover()`. |
