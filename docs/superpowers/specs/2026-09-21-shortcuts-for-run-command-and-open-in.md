...
| D3 | How is that dropdown popped? | Recursive search of the key window's toolbar items for the realized `NSSegmentedControl` whose segment 0 label equals `SavedCommandManager.runButtonTitle`, then `menuForSegment(1)` → `popUp(positioning: nil, at: <control bottom-left>, in: control)`. With **no** saved commands the Run `Menu` carries no `primaryAction:` and is realized as an `NSPopUpButton` instead, matched on its `title` and opened with `performClick(nil)`. | The only handles AppKit exposes; see A6/A7/A13. Matching on the label rules out the other segmented controls in the window (`CommandsView`'s `.pickerStyle(.segmented)` filter, and the Open In split button). The pop-up branch was added in review: without it ⌥⌘R was a silent no-op in the one state the "Run…" row exists for. |
