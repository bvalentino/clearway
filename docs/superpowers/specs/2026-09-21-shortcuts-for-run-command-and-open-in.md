...
| D4 | Which item carries ⌥⌘R? | A **fifth** item, "Run…", directly below "Run <name>" and above the Run submenu. | The task's four-item shape has no home for it: a SwiftUI `Menu` used as a submenu row exposes no action closure that AppKit's key-equivalent dispatch could fire (A5). Flagged to the operator — this adds a row to the enumerated menu. |
