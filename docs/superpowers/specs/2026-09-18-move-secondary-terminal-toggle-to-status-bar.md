...
| 15 | Should the button's hit target be enlarged to the 24x24 frame used elsewhere? | No. It stays the bare 11pt glyph (`ContentViewHelpers.swift:112-120`), roughly 13x11pt. The 24x24 frames belong to the toolbar and the sidebar, which are taller rows; inside this bar the glyph matches every other 11pt clickable item — the path `Text` and both clickable PR states (`ContentViewHelpers.swift:94-106`, `131-150`) — and the bar's verified height leaves no room to grow one item without breaking that rhythm. Raised in the PR review of `dbc3f94`. | Operator |
