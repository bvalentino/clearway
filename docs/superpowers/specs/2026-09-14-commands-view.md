...
| 33 | What happens to a corrupt or unreadable `commands.json`? | `SavedCommandStore.load` still logs a warning and returns empty, but first renames the file to `commands.json.corrupt` beside it, replacing any older `.corrupt`, and logs the rename. The next save then writes a fresh `commands.json` instead of overwriting the only copy of a list the user could still repair by hand. **Supersedes decision 19**, whose "the file is only rewritten when the user next changes something" underestimated how ordinary that next change is — adding one command destroys the file. Raised from the PR review on `add-commands`; see the plan's Changelog C10. | Operator |
| 34 | Does `SavedCommand.Kind` tolerate an unrecognized kind string? | No. The lenient `init(from:)` that mapped an unknown kind to `.terminal` is deleted, so an unrecognized kind is an ordinary decode error: the whole file is unreadable and falls under decision 33, quarantined rather than silently rewritten with a command's kind changed under the user. Supersedes the lenient-kind decision taken in the plan's T1 build log. Raised from the PR review on `add-commands`; see the plan's Changelog C10. | Operator |
| 35 | Does a failed write surface in the UI? | No. `SavedCommandManager.save` stays log-only — `Ghostty.logger.error`, in-memory state untouched — exactly as `WorktreeGroupManager.save` does (`WorktreeGroupManager.swift:225-233`). Reverting the list or raising an alert on a write the user did not ask about is a larger question than this branch, and one answered the same way everywhere else in the app. Raised from the PR review on `add-commands`; see the plan's Changelog C11. | Operator |
