...
| 6 | Is the `guard case .agent` in `TerminalManager.run` removed now that the caller refuses? | No. It guards the *other* `run` overload's shared shape and is the last line of defence on a method reachable from more than the plan path. Two guards at different layers is correct here; this task adds the early one. | Spec author |
