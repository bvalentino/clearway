...
| 8 | Does `stop()` still unlink? | No — nothing in `stop()` unlinks. It unlinks unconditionally today (`AgentActivityMonitor.swift:82`), which is the same theft by a second door: a blocked instance whose toggle is switched off would delete the live instance's socket. The unlink belongs to `HookSocketListener`'s own `deinit`, so holding the listener *is* the "this instance bound it" fact and `stop()` is `listener = nil` and nothing else — a stronger form of the `socketState == .listening` gate this row first named, because it is enforced by a private `init` rather than by a comparison a later edit could get wrong. It is scoped to the inode `bind` created rather than to the name, so a path some other process replaced underneath a running instance survives that instance's teardown as well. | Spec |
| 9 | What does the probe look like to the live instance? | One connection that closes with nothing written, which its accept loop reads as a zero-byte payload and logs as "did not parse" (`AgentActivityMonitor.swift:70-72`). An empty payload is a hang-up, never a hook event, so it is dropped silently. Without that every launch of a second Clearway writes a false malformed-event warning into the first one's log. The drop lives in `acceptPending`, not in the parse guard the row first named: a hang-up is socket-layer knowledge, so an empty read never becomes a payload the monitor has to rule on. | Spec |
...
| 15 | Is the window between the probe and the bind accepted? | Yes. The probe and the `unlink`+`bind` it guards are not atomic, so two Clearways launched at the same moment can both find the path answering nothing, and the one that binds second still replaces the first one's inode — the same theft reached through a race rather than through the rule. Closing it means an `O_EXCL` lock file beside the socket and a policy for the lock a crash leaves behind, which is the `EADDRINUSE` problem again one level up. The case is two builds started within the same few milliseconds, not the daily one the task names, so it is recorded as a known limit beside the saturated backlog (Decision 12), not mitigated. What it does not do is compound: the loser's own teardown unlinks by inode (Decision 8), so the instance whose socket was replaced does not then delete the winner's. | Spec |
...
- **The window between the connect probe and the bind** (Decision 15).
