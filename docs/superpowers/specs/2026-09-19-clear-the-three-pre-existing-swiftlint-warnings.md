...
| 4 | What does `trimmed(_:)` (`:266`) answer for the same bytes? | `""`, via `String(data: data, encoding: .utf8)?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""` — the exact shape at `Worktree.swift:401`. Each caller already stops or concludes safely on an empty result: `trimmed(data) == "true"` falls to `.off` (`:163`), the common dir hits `guard !commonDir.isEmpty` and logs (`:191-195`), and a moved key hits `guard !value.isEmpty else { continue }` (`:201-202`). Returning `String?` would add three nil branches for bytes none of these three commands can emit: `--type=bool` canonicalises to `true`/`false` or exits non-zero, and `--git-common-dir` prints a filesystem path. The third caller is the one that merely skips the key while the bootstrap goes on to enable the extension, and it does not log — it did not before this change either, for genuinely empty output — so it stays as it is and is recorded as a follow-up. | Spec author; rationale corrected at review |
| 5 | Delete `WorktreeDraft.init() {}` as the rule suggests, or suppress the rule? | Suppress, with `// swiftlint:disable:this unneeded_synthesized_initializer` on the existing declaration line (placement per decision 7). The rule triggers on exactly this shape by design — "empty initializers when all properties have defaults" — and its only documented escape is an init with `private`/`fileprivate` access (realm.github.io/SwiftLint/unneeded_synthesized_initializer.html, fetched 2026-09-19), which this init must not have. It cannot see that the init exists to suppress the *memberwise* one, and assumption 5 shows deleting it restores an internal `init(name:branch:branchIsHandEdited:)` that can build a state no mutator can reach. The rule is correctable, so leaving the warning standing also leaves a `--fix` run able to delete the guard. | Spec author |
...
| 7 | Does the doc comment above `init()` stay, and where does the directive go? | The comment stays unchanged. It already states why the initializer exists (`WorktreeDraft.swift:13-16`), which is what makes the suppression legitimate rather than noise. The directive goes **on the declaration line** — `init() {} // swiftlint:disable:this unneeded_synthesized_initializer` — not on a separate `disable:next` line between the comment and the declaration: a `//` line there detaches the `///` block from what it documents and trades the target warning for `orphaned_doc_comment` (observed during T2). Both placements cover exactly one declaration; only this one keeps the comment attached, which is the point of keeping it. This is a comment that "preserves context and prevents a regression" in the sense `CLAUDE.md` allows. | Spec author; placement revised during T2 |
...
| 10 | Does the new non-UTF-8 `return nil` in `values(forWorktreeAt:)` log? | Yes: `log("read \(path)", "git printed bytes that are not UTF-8")`, the same call the `.unavailable` branch three lines below already makes. Every other unreadable-config path in the file is diagnosable — `:104`, `:132`, `:140`, `:169`, `:194` all log before giving up — and this one returns the same `nil` the caller cannot tell apart from a live read, so a silent return would be the file's only undiagnosable failure. | Reviewer recommendation, operator accepted |
...
- `Sources/App/WorktreeConfigStore.swift` — the decoding at `:99` and the `trimmed(_:)` helper at
  `:265-267`.
- `Sources/App/WorktreeDraft.swift` — one trailing directive on the `init()` line.
...
- **`WorktreeDraft`'s state model.** The draft's rules and its tests stay exactly as they are; this
  change appends one trailing comment to `init()` and nothing else.
