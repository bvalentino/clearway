...
| 12 | Where does the view mode parameter go? | **Amended after `ef435f6` (operator decision), overturning the original.** There is no `grouping:` parameter: `WorktreeGroupManager.sidebarOrderedWorktrees` reads the manager's own `grouping` property, exactly as it applies `Worktree.visible` inside itself. The three call sites — the sidebar rows (`SidebarView`), the ⌘N badge (`SidebarView`) and the ⌘1…9 buttons (`ContentView`) — must not be able to disagree about the view mode, and a parameter is what lets them. Purity over inputs was the original argument; it is worth less than one source of truth, and the tests that vary the mode call `setGrouping` first. | Spec author; amended by operator |
