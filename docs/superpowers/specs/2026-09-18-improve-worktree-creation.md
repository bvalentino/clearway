...
| 24 | What decides whether the New Worktree sheet applies the name and status? | The worktree `createWorktree` returns, not `WorktreeManager.error`. A non-fatal fetch failure and a banner left from an earlier refresh both leave `error` set over a creation that succeeded, which silently dropped the name and status the operator had just typed. A returned worktree is applied and the sheet dismisses; no worktree leaves the sheet open with `isCreating` reset, and the case where git reported nothing either is logged. The rule is a pure static, `CreateWorktreeSheet.outcome(created:error:)`, on the precedent of `WorktreeRow.rowTexts` (decision 19). | Operator |
