...
class TerminalManager: ObservableObject {
    ...
    func pane(for worktree: Worktree, app: ghostty_app_t, projectPath: String?) -> TerminalPane {
        ...
        let secondary = Ghostty.SurfaceView(app, workingDirectory: dir, activityOwner: AgentActivityOwner.worktree(key).rawValue)
    ...
    func appendTab(for worktree: Worktree, app: ghostty_app_t, command: String? = nil) -> Ghostty.SurfaceView {
        ...
        if existingPane != nil {
            ...
            ghosttyApp = app
            let secondary = Ghostty.SurfaceView(app, workingDirectory: worktree.path, activityOwner: AgentActivityOwner.worktree(key).rawValue)
    ...
    private func replaceSurface(_ deadSurface: Ghostty.SurfaceView) {
        ...
        for (key, pane) in panes {
            ...
            let newSurface = Ghostty.SurfaceView(app, workingDirectory: dir, activityOwner: AgentActivityOwner.worktree(key).rawValue)
            Self.retireSurface(deadSurface.surfaceId)
            objectWillChange.send()
