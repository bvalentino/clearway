...
enum PortScanner {
    ...
    /// Sorted by `(cwd, port)`, because `proc_listpids` orders by pid: without this, a listener
    /// starting or ending anywhere on the machine — including in a directory no worktree owns —
    /// permutes the array and makes `PortMonitor`'s equality check republish for nothing.
    nonisolated static func scan() -> [Listener] {
        allPids().flatMap { pid -> [Listener] in
        ...
        .sorted { ($0.cwd, $0.port) < ($1.cwd, $1.port) }
    }
    ...
    private static func listeningPorts(pid: pid_t) -> Set<UInt16> {
        ...
        var descriptors = [proc_fdinfo](
            ...
            count: Int(capacity) / MemoryLayout<proc_fdinfo>.size
        )
