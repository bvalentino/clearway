...
Clearway keeps a task backlog per project. **Start Now** on a task creates its worktree, moves the task's `TASK.md` into it, and runs the project's after-create hook in a "Setup" tab beside the worktree's first tab. The task's `status` frontmatter becomes `in_progress` and stays there: Clearway launches no agent of its own, and nothing advances the status for you. (If the task's branch already has a worktree, Start Now just focuses it.) Status is never shown in the app — it is there for whatever reads the file.
