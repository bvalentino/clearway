...
/// Per-entry `command` at the launch sites the coordinator owns: `performLaunch` (autopilot) and the
/// command a step's "Run in New Terminal" stamps onto its launcher tab.
/// The allowlist's own matrix is `AgentLaunchAgentTests`' subject; here the question
