...
final class WorkTaskManagerTests: XCTestCase {
    ...
    // MARK: - rewriteWorktreeReference

    /// Rename happy path: both hidden and exposed tasks linked to the old branch get
    /// rewritten to the new branch; unrelated tasks and unlinked tasks are untouched.
    func testRewriteWorktreeReferenceRewritesHiddenAndExposedTasks() throws {
        let manager = WorkTaskManager(projectPath: tempRoot)

        // Hidden (shadow) task linked to feature/old.
        guard let shadow = manager.createShadowTask(forBranch: "feature/old") else {
            XCTFail("createShadowTask returned nil"); return
        }
        // Second exposed task also linked to feature/old — attach worktree manually so
        // we don't hit `createExposedTask`'s idempotent "find existing by branch" path.
        guard var exposed = manager.createTask(title: "Linked to old") else {
            XCTFail("createTask returned nil"); return
        }
        exposed.worktree = "feature/old"
        manager.updateTask(exposed)

        // Task linked to a different branch — must be untouched.
        guard let unrelated = manager.createExposedTask(forBranch: "feature/other") else {
            XCTFail("createExposedTask returned nil"); return
        }
        // Task with no worktree link — must be untouched.
        guard let orphan = manager.createTask(title: "No branch") else {
            XCTFail("createTask returned nil"); return
        }

        let count = manager.rewriteWorktreeReference(from: "feature/old", to: "feature/new")
        XCTAssertEqual(count, 2, "exactly the two tasks linked to feature/old should be rewritten")

        func loaded(_ id: UUID) -> WorkTask? { manager.tasks.first { $0.id == id } }
        XCTAssertEqual(loaded(shadow.id)?.worktree, "feature/new")
        XCTAssertEqual(loaded(exposed.id)?.worktree, "feature/new")
        XCTAssertEqual(loaded(unrelated.id)?.worktree, "feature/other")
        XCTAssertNil(loaded(orphan.id)?.worktree)

        // Rewrite persists to disk.
        for taskId in [shadow.id, exposed.id] {
            guard let task = loaded(taskId) else { XCTFail("task missing"); return }
            let content = try String(contentsOfFile: manager.filePath(for: task), encoding: .utf8)
            let reparsed = WorkTask.parse(from: content, id: task.id, createdAt: task.createdAt)
            XCTAssertEqual(reparsed?.worktree, "feature/new")
        }
    }

    /// Idempotent: calling again with the already-new branch as `from` finds nothing
    /// tied to the old name and returns 0.
    func testRewriteWorktreeReferenceIsIdempotent() {
        let manager = WorkTaskManager(projectPath: tempRoot)
        _ = manager.createExposedTask(forBranch: "feature/old")

        let first = manager.rewriteWorktreeReference(from: "feature/old", to: "feature/new")
        let second = manager.rewriteWorktreeReference(from: "feature/old", to: "feature/new")

        XCTAssertEqual(first, 1)
        XCTAssertEqual(second, 0, "second pass has no remaining feature/old tasks to rewrite")
    }

    /// Guard: no-op when the from/to names are equal; never reports rewrites.
    func testRewriteWorktreeReferenceNoOpsWhenUnchanged() {
        let manager = WorkTaskManager(projectPath: tempRoot)
        _ = manager.createExposedTask(forBranch: "feature/same")

        let count = manager.rewriteWorktreeReference(from: "feature/same", to: "feature/same")
