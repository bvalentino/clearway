...
- `agent` (`AgentSettings`): `command` (empty/omitted → inherit Settings → Main Terminal at launch via `resolveAgentCommand`; explicit values like `"claude"` / `"grok"` win) + `timeoutMs` (`timeout_ms`, default 600_000 — **decoded but NOT enforced in v1**, like the loop-guard fields). An omitted `agent` falls back entirely to defaults (empty command + default timeout).
