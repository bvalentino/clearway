...
- Default `true` at creation **iff** the project has a valid `WORKFLOW.json` **and the task has content** (`WorkTask.hasContent` — a non-empty title or body). A manually-created worktree with a blank `TASK.md` seeds `autopilot: false` (paused, written explicitly — `nil` would read as on and launch anyway) and its autopilot row is **disabled** until the user gives it something to do — as it also is while the task is a hidden shadow with no current step, since the engine ignores such a worktree and play would have nothing to start. Non-JSON projects have no `autopilot` field.
- Play/pause control: `AutopilotButton` (in `AutopilotButton.swift`) — a full-width `Autopilot: Enabled` / `Autopilot: Disabled` row pinned below the task aside's step cards, outside their `ScrollView`. It carries no gate of its own: `TaskAsideView` renders it under the same `isWorkflowJSONProject` + decoded-definition condition as the cards, and the aside's Task tab is already dropped on main (`SidePanelTab.available(isMain:)`). It keys engine state (`isAgentRunning`, `currentWorkflowStep`) on the worktree **id**, threaded in from `ContentView` as `TaskAsideView.worktreeId` — the branch alone would not resolve those. Click writes `autopilot` via `WorkTaskManager.setAutopilot`. Its disabled state and the copy explaining it derive from **one** `Unavailable?` reason (`nil` while an agent is live, so pause stays reachable), which is what keeps `isDisabled` and the "Unavailable — …" strings from disagreeing: deriving them separately let an enabled, working row announce itself unavailable once a live task was emptied.
...
### Loop guard / stopping a step

v1 has **no automatic attempt cap** (a single `attempt` counter couldn't bound a real fix↔test loop given the `S == P` ignore rule; `maxAttempts`/`onMaxAttempts` remain reserved/unenforced), and **no in-app stop control**. The v1 loop-stopper is the agent's own terminal: Ctrl-C in it, or closing its tab, exits the agent mid-step, and `handleChildExited` → `pauseIfAgentDiedMidStep` pauses autopilot so the engine's idle rule doesn't respawn the same action on the next reload. (A manual status pick also terminates a running agent, but it *steers* the loop to the picked action without pausing autopilot.)
