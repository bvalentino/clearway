...
- **Manual status pick.** The task aside's status picker lists the `WORKFLOW.json` actions (`workflowActionSlugs()`, flow-ordered) and writes via `setWorkflowStatus` — a human pick clears any halt + error first so the loop recovers from a halted state, then the watcher launches the picked action (or the pause gate holds it when `autopilot: false`).
