...
- Play/pause control: `AutopilotButton` (in `AutopilotButton.swift`) — a full-width `Autopilot: Enabled` / `Autopilot: Disabled` row pinned below the task aside's step cards, outside their `ScrollView`. It carries no gate of its own: `TaskAsideView` renders it under the same `isWorkflowJSONProject` + decoded-definition condition as the cards, and the aside's Task tab is already dropped on main (`SidePanelTab.available(isMain:)`). It keys engine state (`isAgentRunning`, `currentWorkflowStep`) on the worktree **id**, threaded in from `ContentView` as `TaskAsideView.worktreeId` — the branch alone would not resolve those. Click writes `autopilot` via `WorkTaskManager.setAutopilot`.
